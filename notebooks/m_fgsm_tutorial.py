...
def print_info(x):
    ...
    if isinstance(x, list) or isinstance(x, np.ndarray):
        return str(np.shape(x)) + ", " + str(np.array(x).dtype)
...
log = log + "cell [3] OUT, pretrained_model, " + print_info(pretrained_model) + "\n"
log = log + "cell [3] OUT, epsilons, " + print_info(epsilons) + "\n"

# In[8]:log = log + "cell [4] IN, use_cuda, " + print_info(use_cuda) + "\n"
log = log + "cell [4] IN, pretrained_model, " + print_info(pretrained_model) + "\n"
...
# 
# 
f = open("m_fgsm_tutorial_log.txt", "w")
f.write(log)
f.close()
